...
void loop()
{
    ...
    if ( currentTime - previousTimeLoop >= onTime)
    {
        ...
        for ( int t = 0; t < 8; t++)
        {
            if ((diff[t] > tol) && (flag[0] == 0) && (CurrentPack >= 0.2))
            {
...
void SerialMonitor()
{
    if (currentTime - previousTimeDisplay >= 1000)
    {
        ...
        while (Wire1.available())
        {
            Ah_Pack[0] = Wire1.read();
            Ah_Pack[1] = Wire1.read();
            Ah_Pack[2] = Wire1.read();
            Ah_Pack[3] = Wire1.read();
            Ah_Pack[4] = Wire1.read();

            AhPack = atof(Ah_Pack);
            Serial.println(AhPack);
        }

        if (CurrentPack < 0.00)
        {
            Runtime = (AhPack/(-1.00*CurrentPack)) * 60.00;
        }
...
void LowestCellAlgorithm()
{
    ...
    /* Find the index of the lowest cell value */
    int index = 0;
    int count = sizeof(BAT) / sizeof(BAT[0]);
    ...
    diff[0] = BAT[0] - BAT[index];
    diff[1] = BAT[1] - BAT[index];
    diff[2] = BAT[2] - BAT[index];
    diff[3] = BAT[3] - BAT[index];
    diff[4] = BAT[4] - BAT[index];
    diff[5] = BAT[5] - BAT[index];
    diff[6] = BAT[6] - BAT[index];
    diff[7] = BAT[7] - BAT[index];
}
...
void ResetButton()
{
   if (digitalRead(A1) < 1.00 && (currentTime - previousTimeReset >= 250))
   {
      Wire1.beginTransmission(2);
      Wire1.write('R');
      Wire1.endTransmission();
...
// ex. NodNum, 101, UVP 
//     1, 101, 2.70
// By default the NodNum is set to 1
//------------------------------------------------------------------------------
// List of commands
//------------------------------------------------------------------------------
// 1. 1,10          | Resets two point calibration values
//------------------------------------------------------------------------------
// 2. 1,11          | Blank Function 
//------------------------------------------------------------------------------
// 3. 1,12,3.00,6.00, | Two point calibration for the adc values.  First set 
...
//                    | cell8
//------------------------------------------------------------------------------
// 4. 1,100,0.015   | Sets new delta between the lowest cell and the rest of the 
//                  | cells.  The example on the left sets the tolerance to 15mV
//------------------------------------------------------------------------------
// 5. 1, 101, 2.70   | Sets new under voltage threshold value
//------------------------------------------------------------------------------
// 6. 1, 102, 3.75   | Sets new over voltage threshold value
//------------------------------------------------------------------------------
// 7. 1, 103, 1000   | Sets new value for main loop interval
//------------------------------------------------------------------------------
// 8. 1, 1000        | Resets fault condition and returns system to normal
//------------------------------------------------------------------------------
// 9. 1, 1001        | Test function (not used)
// 10. 1, 2000       | Shows you current cell voltages and current values
////////////////////////////////////////////////////////////////////////////////
void serialEvent()
{
